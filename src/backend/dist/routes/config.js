...
const configRoutes = async (fastify) => {
    ...
    fastify.post('/codebase', {
        ...
        try {
            const { githubConfig, sharepointConfig, ignisConfig } = request.body;
    ...
    fastify.post('/github/validate', {
        ...
        try {
            const { repositoryUrl, token } = request.body;
        ...
        try {
            ...
            const status = {
                github: config.githubConfig ?
                    await fastify.checkGitHubStatus(config.githubConfig) :
                    { status: 'not_configured' },
                sharepoint: config.sharepointConfig ?
                    await fastify.checkSharePointStatus(config.sharepointConfig) :
                    { status: 'not_configured' },
                ignis: config.ignisConfig ?
                    await fastify.checkIgnisStatus(config.ignisConfig) :
                    { status: 'not_configured' }
            };
            return status;
        }
    ...
    fastify.post('/test-all', {
        ...
        try {
            ...
            if (config.githubConfig) {
                results.github = await fastify.validateGitHubConfig(config.githubConfig);
                if (!results.github.valid)
                    results.overallStatus = 'partial';
            }
            // Test SharePoint if configured
            if (config.sharepointConfig) {
                results.sharepoint = await fastify.validateSharePointConfig(config.sharepointConfig);
                if (!results.sharepoint.valid)
                    results.overallStatus = 'partial';
            }
            // Test Ignis if configured
            if (config.ignisConfig) {
                results.ignis = await fastify.validateIgnisConfig(config.ignisConfig);
                if (!results.ignis.valid)
                    results.overallStatus = 'partial';
            }
            return results;
        }
